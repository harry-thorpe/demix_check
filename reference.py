...
def setup_reference(mash_exec, themisto_build_exec, d, t, ss, thr_prop_min, thr_prop_exp):
...
def get_thresholds(in_clu, in_dis, thr_prop_min, thr_prop_exp, out_file):
    ...
    dis_diff=dis_diff.groupby("ref_cluster", as_index=False).agg(
            dis_diff_cluster_med=("distance", "median"),
            dis_diff_cluster_min=("distance", "min"))

    dis=dis.query("ref_cluster == met_cluster")
    dis=dis.query("ref_id != met_id")
    dis=dis.groupby("ref_cluster", as_index=False).agg(
            dis_med=("distance", "median"),
            dis_min=("distance", "min"),
            dis_max=("distance", "max"))
    
    dis=dis.merge(dis_diff, how="left", on="ref_cluster")

    dis["threshold"]=dis["dis_max"]*(1+thr_prop_exp)
    dis["threshold"]=np.where(dis["threshold"] > dis["dis_diff_cluster_min"], dis["dis_max"], dis["threshold"])
    dis=dis.rename(columns={"ref_cluster":"cluster"})
    dis=dis[["cluster", "threshold"]]
    
    t_min=np.median(dis_diff["dis_diff_cluster_med"])*thr_prop_min
